...
(function() {
  ...
  const ui = {
    ...
    guestSelectedGame: document.getElementById('guest-selected-game'),
    selectGameType: document.getElementById('select-game-type'),
    cfgSpeedNumbers: document.getElementById('cfg-speed-numbers'),
    cfgWordGames: document.getElementById('cfg-word-games'),
    cfgCategorySelect: document.getElementById('cfg-category-select'),
    selectMaxNumbers: document.getElementById('select-max-numbers'),
    selectWordRounds: document.getElementById('select-word-rounds'),
  ...
  function initGame(firstAttackerIsHost) {
    ...
    // Generar tablero de búsqueda propio usando Web Worker
    if (typeof Worker !== 'undefined') {
      const boardWorker = new Worker('boardWorker.js');
      boardWorker.postMessage({ maxNumbers: state.maxNumbers });
      boardWorker.onmessage = (e) => {
        state.myBoard = e.data.board;
        renderSearchBoard();
      };
    } else {
      // Fallback si el navegador no soporta Web Workers
  ...
  function setupEventListeners() {
    ...
    ui.selectGameType.addEventListener('change', () => {
      ...
      if (val === 'MIND_SAME') gameTitle = '🔮 Telepatía Total';

      window.netManager.send({
        type: 'GAME_TYPE_CHANGE',
        gameType: val,
        gameTitle: gameTitle
      });
    });

    // Configuración de Meta Caza Números (Host)
    ...
    ui.btnStartGame.addEventListener('click', () => {
      const gameType = ui.selectGameType.value;

      if (gameType === 'SPEED_NUMBERS') {
        state.maxNumbers = parseInt(ui.selectMaxNumbers.value, 10);
        ...
        // Juegos de palabras: MIND_DIFF o MIND_SAME
        ...
        window.wordGames.startGame(gameType, rounds);
      }
    });
    ...
    window.netManager.on('game_type_changed', (data) => {
      if (ui.guestSelectedGame) {
        ui.guestSelectedGame.textContent = 'Juego: ' + data.gameTitle;
      }
    });

    window.netManager.on('start_word_game', (data) => {
      // Store explicit category index (or -1 for random) on the controller before starting
      if (window.wordGames) {
        window.wordGames.explicitCategoryIdx = typeof data.selectedCategory === 'number' ? data.selectedCategory : -1;
